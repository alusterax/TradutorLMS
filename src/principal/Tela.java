...
public class Tela extends Shell {
	...
	public Tela(final Display display) {
		...
		mntmAbrir.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				if(salvo==false){
					int resp = JOptionPane.showConfirmDialog(null, "O programa não foi salvo, deseja fecha-lo mesmo assim?");
					if(resp == 0){
						try{
	...
	public static void Lexico(){
		...
		for(int i=0; i<ALfinal.size(); i++){
			String[] a = {(i+1+""), ALfinal.get(i).getCodigo()+"", ALfinal.get(i).getNome(), ALfinal.get(i).getDesc()};
